    ...
    public partial class Reservation : Form
    {
                ...
                else
                {
                    ...
                    if (dir > 0)//N aswan ->alex
                    {
                        ...
                        for (int i = 0; i <= dtt2.Rows.Count - 1; i++)
                        {
                            ...
                            //last update
                            var curfrom = direction[dtt2.Rows[i][5].ToString()];//12
                            var curto = direction[dtt2.Rows[i][6].ToString()];//5

                            int fromnum = direction[cnbfrom.Text];//13
                            int tonum = direction[cmbto.Text];//12
                            bool k = false;
                            for(int j = fromnum; j >= tonum; j--)
                            {
                                if (j > curto && j < curfrom) k = true;
                            }
                            if (fromnum >= curfrom && tonum <= curto) k = true;
                            if (comseat.Items.Contains(curseat.ToString()) && curdate.ToString() == datee && trainid2 == curtid.ToString() &&k==true)
                            {
                    ...
                    else if (dir < 0)
                    {
                        try
                        {
                            ...
                            for (int i = 0; i <= dtt2.Rows.Count - 1; i++)
                            {
                                ...
                                for (int j = fromnum; j <= tonum; j++)
                                {
                                    if (j > curfrom && j < curto) k = true;
                                }
                                if (fromnum <= curfrom && tonum >= curto) k = true;
                                if (k==true&&comseat.Items.Contains(curseat.ToString()) && curdate.ToString() == datee && trainid2 == curtid.ToString())
                                {
                                    comseat.Items.Remove(curseat.ToString());
                                }
                            }
        ...
        private void btnreserve_Click(object sender, EventArgs e)
        {
            ...
            if (seatid == "") MessageBox.Show("Please Selcet Seat Number");
                ...
                DateTime residing_time3 = new DateTime(residing_time2.Year, residing_time2.Month, residing_time2.Day, residing_time1.Hour, residing_time1.Minute, residing_time1.Second);                
